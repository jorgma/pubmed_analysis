...
def parse_pubmed_web_tree(tree):
    ...
    if len(pubdate) >= 1 and pubdate[0].find("year") is not None:
        ...
        # added by lyj:
        month = pubdate[0].find("month").text
        day = pubdate[0].find("day").text
    ...
    countries = tree.xpath("//country")
    if len(countries) >= 1:
        country = countries[0].text

    publication_types = tree.xpath("//publicationtype")
    if len(publication_types) >= 1:
        publication_type = publication_types[0].text
